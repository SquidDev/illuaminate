...
let count_nothing = [ note ~tag "Numeric for loop has a step of 0. It will never progress." ]

let count_up ~start ~limit =
  [ note ~tag "Numeric for loop counts up from %.14g to %.14g, but has a negative step." start limit
  ]

let count_down ~start ~limit =
  [ note ~tag "Numeric for loop counts down from %.14g to %.14g, but has a non-negative step." start
      limit
  ]
...
let stmt () _ = function
  | ForNum { forn_start; forn_limit; forn_step; _ } -> (
      ...
      | Some start, Some limit, Some step when limit > start && step < 0.0 -> count_up ~start ~limit
