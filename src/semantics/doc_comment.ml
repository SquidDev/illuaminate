...
module Link = struct
  let of_tag tags label =
    ...
    | [ ("link", Some link); ("style", Some style) ] ->
        ...
        in
        { link_reference = Reference link; link_label = Description label; link_style = style }
    | _ -> invalid_arg "malformed illuaminate:ref"

  let to_tag { link_reference = Reference link; link_label = Description label; link_style } :
      Omd.element =
    let style =
      ...
      | `Text -> "text"
    in
    Html ("illuaminate:ref", [ ("link", Some link); ("style", Some style) ], label)
end
